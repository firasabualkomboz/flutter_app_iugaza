...
void main() {
  testWidgets(
      'Test Home Widget by checking if the title is present on app start',
      (WidgetTester tester) async {
    await tester.pumpWidget(MaterialAppTester(Home()));

    expect(find.text('FlutterTodo'), findsOneWidget);
  });

  testWidgets(
      'Test new empty NewTodo Widget by checking if the title is present and the item is null',
      (WidgetTester tester) async {
    final newTodoView = new NewTodoView();

    await tester.pumpWidget(MaterialAppTester(newTodoView));
  ...
  testWidgets(
      'Test NewTodo Widget with an item by checking if the title is present, the item was loaded and set on the textfield',
      (WidgetTester tester) async {
